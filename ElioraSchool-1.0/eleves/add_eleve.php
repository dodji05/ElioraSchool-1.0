...
if (isset($_POST['is_posted']) and ($_POST['is_posted'] == "1")) {
	...
	$reg_lieu_naissance = trim($reg_lieu_naissance);
...
echo "<td>\n";
	...
	echo "<tr>\n";
		...
		if (isset($eleve_login)) {
		    echo "<td>Identifiant ElioraSchool * : </td>
	...
	<tr>
		<td>Téléphone : </td>
		<td><input type=text name='reg_telephone' id='reg_telephone' size=20 <?php if (isset($eleve_telephone)) { echo "value=\"".$eleve_telephone."\"";}?> /></td>
	</tr>
	<tr>
		<td>Lieu de naissance : </td>
		<td><input type=text name='reg_lieu_naissance' id='reg_lieu_naissance' size=20 <?php if (isset($eleve_lieu_naissance)) { echo "value=\"".$eleve_lieu_naissance."\"";}?> /></td>
	</tr>
...
if ($nombreligne != 0) {
	...
	$i = 0;
	//while ($i < $nombreligne){
	while($lig_resp1=mysqli_fetch_object($call_resp)){
	...
	if($eleve_no_resp2!=0){
		...
		$res_resp2=mysqli_query($GLOBALS["mysqli"], $sql);
		if(mysqli_num_rows($res_resp2)>0){
			$lig_resp2=mysqli_fetch_object($res_resp2);
...
<?php
/*
$calldata = mysqli_query($GLOBALS["mysqli"], "SELECT * FROM etablissements ORDER BY id");
