...
$sql_classe = "SELECT * FROM classes";
$res_classe = mysqli_query($GLOBALS["mysqli"],$sql_classe);
$nbresultats = mysqli_num_rows($res_classe);
if($nbresultats >0)
{
// il existe au moins une classe on les affiche
    echo"<ul>";
        $k=0;
        while ($k < $nbresultats){
            echo"<li>";
            $nom_classe = old_mysql_result($res_classe,$k,'classe');
            $idclasse = old_mysql_result($res_classe,$k,'id');
            echo $nom_classe;
            $sql_tranche = "SELECT * FROM etg_tranche t,etg_fraisscolaire f WHERE ((t.id_frais =f.idFrais) AND( f.id_classe =$idclasse )and ( Date_Fin_Tranche BETWEEN DATE_SUB( Date_Fin_Tranche, INTERVAL 21 DAY) AND NOW())) ";
          // $sql_tranche= "SELECT * FROM `etg_tranche` WHERE Date_Fin_Tranche BETWEEN DATE_SUB( Date_Fin_Tranche, INTERVAL 21 DAY) AND NOW()";
            $res_tranche = mysqli_query($GLOBALS["mysqli"],$sql_tranche);
          // echo $sql_tranche;

            //Affichage des tranches par classe
            $nbtranches = mysqli_num_rows($res_tranche);
            $i=0;
            echo"<ul>";
            while($i< $nbtranches)
            {
                echo "<li>";
                    echo old_mysql_result($res_tranche,$i,'Nom_Tranche')." - ".old_mysql_result($res_tranche,$i,'Date_Fin_Tranche')." - ". old_mysql_result($res_tranche,$i,'Montant_Tranche');;
