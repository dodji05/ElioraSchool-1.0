...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	...
	if ($droits == "V") {
		echo " | <a href=\"import_eleves_csv.php\" title=\"Télécharger le fichier des noms, prénoms, identifiants ".$AppName." et classes\">Télécharger le fichier des élèves au format csv.</a>\n";
	}
...
if ($test == '0') {
	echo "<p class='grand'>Attention : il n'y a aucun élève dans la base ".$AppName." !</p>\n";
	if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
...
//} else {
  ...
  //echo "Emplacement de la table par defaut";
if(isset($quelles_classes)) {
	echo "$quelles_classes<br />";

	echo "<p class='small'><em>Remarque&nbsp;:</em> l'identifiant mentionné ici ne permet pas aux élèves de se connecter à ".$AppName.", il sert simplement d'identifiant unique.";
	//if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	if($_SESSION['statut']=="administrateur") {
		echo " Pour permettre aux élèves de se connecter à ".$AppName.", vous devez leur créer des comptes d'accès, en passant par la page Gestion des bases -> Gestion des comptes d'accès utilisateurs -> <a href='../utilisateurs/edit_eleve.php'>Elèves</a>.";
	}
...
?>
   ...
   // echo" Par defaut lignesee";
    $order_type='nom,prenom';
    $quelles_classes = "toutes";
    $csv="";
