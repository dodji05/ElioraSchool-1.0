...
//include_once("header_releve_pdf.php");
...
if (isset($_POST["versement"])){
    ...
    require_once ("recu_paiement.php");*/
}
    ...
    $sql_tranche = "SELECT * FROM etg_fraisscolaire, etg_tranche where((etg_fraisscolaire.idFrais = etg_tranche.id_Frais) And( id_classe=$id_classe))";
...
?>
    <form method="post" action="<?php echo $_SERVER['PHP_SELF']?>">
        ...
        <table border="1" class='boireaus' cellpadding='2'summary='Tableau des élèves de la classe'>
            ...
            $alt=1;
            $rest_a_payer =array();
            $pt = old_mysql_result($calltranche, 0, "Montant_Tranche");
            $reste = $pt - $total;
            if($reste == 0 ){
                $rest_a_payer[0] =0;
                for($j=1; $j < $nombreligne; $j++){
                    $rest_a_payer[$j] =old_mysql_result($calltranche, $j, "Montant_Tranche");
                }
            }
            else if ($reste > 0) {
               ...
               /* echo */$reste1 =($reste)*(-1);
                for($j=1; $j < $nombreligne; $j++){
                    $rst= old_mysql_result($calltranche, $j, "Montant_Tranche");
                   /* echo "reste".*/$reste = $rst - $reste1."<br/>";
                    if($reste == 0 ){
                        $rest_a_payer[$j] =0;
						$reste1 = 0;
						for($e=$j+1; $e<$nombreligne; $e++){
						$rest_a_payer[$e] =old_mysql_result($calltranche, $e, "Montant_Tranche");
						}
						break;
                    }
                    else if ($reste > 0) {
          ...
          //  print_r($rest_a_payer);
			...
			$alt=$alt*(-1);
                ...
                echo "<td>";
                echo old_mysql_result($calltranche, $i, "Nom_Tranche");
                echo "</td>";

                echo "<td>";
                echo number_format(old_mysql_result($calltranche, $i, "Montant_Tranche") , 0, ',', ' ');
                echo "</td>";

                echo "<td>";
                $rt = @old_mysql_result($calltranche, $i, "Montant_Tranche");
                ...
                echo "<td>";
                echo old_mysql_result($calltranche, $i, "Date_Fin_Tranche");
                echo "</td>";
...
echo "DETAILS DE PAYEMENT DES FRAIS SCOLARITE :";
$sql_list_paye ="SELECT * FROM etg_paiement WHERE(idEleve=$id_eleve and idLogin='$el_login')";
