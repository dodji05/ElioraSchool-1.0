...
if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")) {
	...
	if ($droits == "V") {
		echo " | <a href=\"import_eleves_csv.php\" title=\"Télécharger le fichier des noms, prénoms, identifiants ElioraSchool et classes\">Télécharger le fichier des élèves au format csv.</a>\n";
	}
...
if ($test == '0') {
	echo "<p class='grand'>Attention : il n'y a aucun élève dans la base ElioraSchool !</p>\n";
	if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
...
if (!isset($quelles_classes)) {
	...
	if($_SESSION['statut'] == 'professeur') {
		...
		echo "<form enctype='multipart/form-data' action='payement_frais.php' method='post' name='formulaire'>\n";
...
Mettre à jour votre table etablissements peut être une solution.\">Vide ou non référencée</option>";
		...
		if(mysqli_num_rows($test_na)==0){
			...
			echo "<span class='norme'>Les élèves non affectés à une classe (<i>".mysqli_num_rows($test_na)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
...
// Eric Les élèves dont la date de sortie de l'établissement est renseignée      
		...
		if(mysqli_num_rows($test_dse)==0){
			...
			echo "<span class='norme'>Les élèves dont la date de sortie de l'établissement est renseignée (<i>".mysqli_num_rows($test_dse)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
			...
			if(mysqli_num_rows($test_dse2)>0){
				...
				echo "<span class='norme'>Les élèves dont la date de sortie de l'établissement est renseignée et qui sont pourtant inscrits dans une classe (<i>".mysqli_num_rows($test_dse2)."</i>).</span><br />\n";
		...
		if(mysqli_num_rows($test_incomplet)==0){
			...
			echo "<span class='norme'>Les élèves dont l'Elenoet ou le Numéro national (INE) n'est pas renseigné (<i>".mysqli_num_rows($test_incomplet)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		*/
		...
		if(mysqli_num_rows($test_no_etab)==0){
			...
			echo "<span class='norme'>Les élèves dont l'établissement d'origine n'est pas renseigné (<i>".mysqli_num_rows($test_no_etab)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if(mysqli_num_rows($test_incomplet)==0){
			...
			echo "<span class='norme'>Les élèves dont l'email n'est pas renseigné dans la table 'eleves' (<i>".mysqli_num_rows($test_incomplet)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		...
		if(mysqli_num_rows($test_sans_compte)==0){
			...
			echo "<span class='norme'>Les élèves qui n'ont pas de compte d'utilisateur (<i>".mysqli_num_rows($test_sans_compte)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
		...
		if(mysqli_num_rows($test_compte_inactif)==0){
			...
			echo "<span class='norme'>Les élèves disposant d'un compte d'utilisateur, mais dont le compte est inactif (<i>".mysqli_num_rows($test_compte_inactif)."</i>).</span><br />\n";
			echo "</label>\n";
			echo "</td>\n";
			echo "</tr>\n";
		}
...
if(isset($quelles_classes)) {
	...
	echo "<p class='small'><em>Remarque&nbsp;:</em> l'identifiant mentionné ici ne permet pas aux élèves de se connecter à ElioraSchool, il sert simplement d'identifiant unique.";
	//if(($_SESSION['statut']=="administrateur")||($_SESSION['statut']=="scolarite")){
	if($_SESSION['statut']=="administrateur") {
		echo " Pour permettre aux élèves de se connecter à ElioraSchool, vous devez leur créer des comptes d'accès, en passant par la page Gestion des bases -> Gestion des comptes d'accès utilisateurs -> <a href='../utilisateurs/edit_eleve.php'>Elèves</a>.";
	}
	elseif($_SESSION['statut']=="scolarite") {
		echo " Pour permettre aux élèves de se connecter à ElioraSchool, vous devez vous connecter en 'administrateur' et leur créer des comptes d'accès, en passant par la page Gestion des bases -> Gestion des comptes d'accès utilisateurs -> Elèves.";
	}
	echo "</p>\n";
...
/*
	...
	echo "<p>Remarque : le login ne permet pas aux élèves de se connecter à ElioraSchool. Il sert simplement d'identifiant unique.</p>\n";
*/
	...
	while ($i < $nombreligne){
		if(!isset($tab_eleve[$i])){
			//print_r($calldata);
			$eleve_login = old_mysql_result($calldata, $i, "login");
			$id_eleve = old_mysql_result($calldata, $i, "id_eleve");
		...
		if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')) {
			 echo "<td><p>";
           echo "<a href='payement_traite.php?id_eleve=$id_eleve&el_login=$eleve_login&idclasse=$eleve_id_classe' >";
         //   echo "<span id='prof_suivi_$eleve_login'>";
            echo "effectuer un payement";
            echo "</span>";
            echo "</a>";
            echo "</p></td>\n";
		}
	...
	echo"<h2>Apprenant ayant une écheanchier</h2>";
    include("index_call_data_echeance.php");
...
//    echo "<th><p>Classe</p></th>";
    ...
    */
    ...
    while ($i < $nombreligne){
        ...
        if(($_SESSION['statut']=='administrateur')||($_SESSION['statut']=='scolarite')) {
            echo "<td><p>";
            echo "<a href='echeance_payement_traite.php?id_eleve=$id_eleve&el_login=$eleve_login&idclasse=$eleve_id_classe' >";
            //   echo "<span id='prof_suivi_$eleve_login'>";
            echo "effectuer un payement";
            echo "</span>";
            echo "</a>";
