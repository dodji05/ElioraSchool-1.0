	...
	if((getSettingAOui('utiliser_phpmailer'))&&(getSettingValue('phpmailer_smtp_host')!="")&&(getSettingValue('phpmailer_smtp_port')!="")&&(getSettingValue('phpmailer_from')!="")) {
		...
		*/
		...
		if((isset($tab_param_mail['from']))&&(check_mail($tab_param_mail['from']))) {
			...
			$mail->FromName = 'Mail automatique ElioraSchool';
		}
		...
		*/
		...
		//	$AppName = "ElioraSchool";
			...
			$headers .= "From: Mail automatique ".$AppName." <ne-pas-repondre@".$_SERVER['SERVER_NAME'].">\r\n";
...
function tentative_intrusion($_niveau, $_description, $_login_a_enregistrer="") {
	...
	// Si la tentative d'intrusion a été effectuée par un utilisateur connecté à ElioraSchool,
	...
	if ($send_email) {
		$message = "** Alerte automatique sécurité ".$AppName." **\n\n";
		$message .= "Une nouvelle tentative d'intrusion a été détectée par ".$AppName." Les détails suivants ont été enregistrés dans la base de données :\n\n";
		$message .= "Date : ".$date."\n";
		...
		$subject = $gepiPrefixeSujetMail."".$AppName." : Alerte sécurité -- Tentative d'intrusion";
		$subject = "=?UTF-8?B?".base64_encode($subject)."?=\r\n";
 ...
 * 
 * Test le dossier en écriture et le crée au besoin
 * La fonction est appelée depuis la racine de l'arborescence ElioraSchool (sinon ça peut bugger)
 *
		...
		if ($res_user->num_rows > 0) {
			$lig_user = $res_user->fetch_object();
			 if(check_mail($lig_user->email)) {
				...
				$message = "** Mail connexion ".$AppName." **\n\n";
				$message .= "\n";
				$message .= "Vous (*) vous êtes connecté à ".$AppName." :\n\n";
...
function mail_alerte($sujet,$texte,$informer_admin='n') {
	...
	if ($res_user->num_rows > 0) {
		...
		//$message = "** Mail connexion ElioraSchool **\n\n";
		$message=$texte;
		$message .= "\n";
		$message .= "Vous (*) vous êtes connecté à ElioraSchool :\n\n";
		$message .= "Identité                : ".mb_strtoupper($lig_user->nom)." ".ucfirst(mb_strtolower($lig_user->prenom))."\n";
		$message .= "Login                   : ".$user_login."\n";
		$message .= "Date                    : ".$date."\n";
		$message .= "Origine de la connexion : ".$adresse_ip."\n";
...
function liste_prof_suivi($id_classe, $pour_qui="", $avec_lien_mail="") {
	...
	for($loop=0;$loop<count($tab);$loop++) {
		...
		if($mail_user!="") {
			if($pour_qui=="profs") {
				$retour.="<a href='mailto:$mail_user?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail'))."[ElioraSchool]: ...' title=\"Envoyer un mail.\">".civ_nom_prenom($tab[$loop])."</a>";
			}
			else {
				$retour.="<a href='mailto:$mail_user?".urlencode("subject=".getSettingValue('gepiPrefixeSujetMail'))."[ElioraSchool]: ...' title=\"Envoyer un mail.\">".affiche_utilisateur($tab[$loop], $id_classe)."</a>";
			}
		}
		else {
			if($pour_qui=="profs") {
 ...
 * @param int $niveau_arbo Niveau dans l'arborescence ElioraSchool
