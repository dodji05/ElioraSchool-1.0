 ...
 * - $tbs_ElioraSchoolSchoolName : nom de l'établissement
 * - $tbs_message_enregistrement
 * - $tbs_temps_max : temps maximal de session
 * - $tbs_ElioraSchoolPath : chemin de ElioraSchool
 * - $tbs_prototype : fichier de la bibliothèque prototype
 * - $tbs_charger_observeur
 * - $tbs_degrade_entete
 * - tbs_modif_bandeau
 * - $tbs_bouton_taille : chemin de ElioraSchool si on affiche les boutons de réduction de taille du bandeau
 * - $titre_page
 * - $tbs_nom_prenom
 * - $tbs_last_connection
 * - $tbs_mise_a_jour - chemin de ElioraSchool si le module mise à jour est actif
 * - $tbs_version_ElioraSchool
 ...
 * @todo $prefix et $ElioraSchoolPath2="."; déjà défini dans initialisation.inc.php, on peut économiser un test
 */
...
// ======================= Début de l'affichage des feuilles de style 55555555555555555555555================
...
//=== statut utilisateur ===
	...
	*/
	...
	if (isset($_SESSION['statut'])) {
		if ($_SESSION['statut'] == 'administrateur') {
			$tbs_deux_menu[]=array("lien"=>"#" , "onclick"=> "onclick=\"window.open(this.href, '_blank'); return false;\""  , "texte"=>"Visiter le site de ElioraSchool");
		}else{
