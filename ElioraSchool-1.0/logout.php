...
//$message = "<h1 class='gepi'>Déconnexion</h1>";
    ...
    if (!isset($_GET['auto']) || !$_GET['auto']) {
    	$session_gepi->close(0);
        $message .= "Vous avez fermé votre session ElioraSchool.";
        ...
        if (sql_query1($sql)) {
           ...
           $session_gepi->close($_GET['auto']);
           $message .= "Votre session ElioraSchool a expiré car le temps maximum (".getSettingValue("sessionMaxLength")." minutes) sans échange avec le serveur a été atteint.<br /><br />Date et heure de la déconnexion : ".$date_fermeture."";
           //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
        } else {
           $message .= "<h1 class='gepi'>Fermeture d'une fenêtre ElioraSchool</h1>";
           $titre= "Fermeture d'une fenêtre ElioraSchool";
           /*
           ...
           */
			$message .= "A l'heure ci-dessous, une fenêtre ElioraSchool s'est automatiquement fermée par mesure de sécurité. Le temps maximum de ".getSettingValue("sessionMaxLength")." minutes sans échange avec le serveur a sans doute été atteint.<br /><br />
           Heure et date de fermeture de la fenêtre : ".$date_fermeture;
           //$message .= "<a href=\"$gepiPath/login.php\">Ouvrir une nouvelle session</a>.";
        }
    } else {
        $session_gepi->close($_GET['auto']);
        $message .= "Votre session ".$AppName." a expiré, ou bien vous avez été déconnecté.<br />";
